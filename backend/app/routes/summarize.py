...
async def summarize_endpoint(req: SummarizeRequest, request: Request, user=Depends(get_current_user), token: HTTPAuthorizationCredentials = Depends(security)):
    """
    ...
    if len(req.text) > 100_000:
        raise HTTPException(413, detail="Text too long")
